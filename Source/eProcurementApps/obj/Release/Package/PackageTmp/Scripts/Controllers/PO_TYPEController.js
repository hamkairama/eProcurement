...
function BtnAction(action) {
    ...
    if (IsValidForm() == 0) {
        if (IsValidDate() == 0) {
            if (IsValidLength() == 0) {
                var id = $.xResponse(fullOrigin + '/IsInActive/', { value: $("#txt_po_type_name").text() });
                if (id > 0) {
...
function CheckData(action) {
    txt_po_type_name = $("#txt_po_type_name").text();
    txt_id = 0;
    if (action == "edit") {
        txt_id = $("#txt_id").text();
    };
    $.ajax({
        ...
        success: function (data) {
            var field = document.getElementById('required_txt_po_type_name');
            if (data == 1) {
...
function ActionCreate() {
    txt_po_type_name = $("#txt_po_type_name").text();
    txt_po_type_description = $("#txt_po_type_description").text();
    ...
    $.ajax({
        ...
        data: {
            po_type_name: txt_po_type_name,
            po_type_description: txt_po_type_description,
