...
window.onload = function () {
    link_eproc = $("#txt_link_eproc").text();;
};
        ...
        if (IsValidDate() == 0) {
            if (IsValidLength() == 0) {
                if (IsOnlyNumberInput() == 0) {
                    var id = $.xResponse(link_eproc + 'APPROVALROLE/IsInActive/', {
                        value: $("#txt_approvalrole_number").text()
                    });
                    if (id > 0) {
...
function CheckData(action) {
    var txt_wa_number = $("#txt_approvalrole_number").text();
    var txt_id = 0;
    if (action == "create") {
        $.ajax({
            url: '../APPROVALROLE/CheckData',
            type: "Post",
            data: {
                id: txt_id,
            ...
            success: function (data) {
                var field = document.getElementById('required_txt_approvalrole_number');
                if (data == 1) {
...
function ActionCreate() {
    var txt_wa_number = $("#txt_rolename").text(); 
...
function ActionDelete() {
    var txt_id = $("#txt_id").text();
    $.ajax({
        url: link_eproc + 'WA/ActionDelete',
        type: 'Post',
        data: {
            id: txt_id,
        },
...
function ActionActivating(txt_id) {
    $.ajax({
        url: link_eproc + 'WA/ActionActiviting',
        type: 'Post',
        data: {
            id: txt_id,
        },
...
function CheckDataRequestWA(action) {
    ...
    var txt_id = 0;
    var urlLink = link_eproc + 'REQUEST/CheckData';
...
function ActionRequestWACreate() {
    ...
    if (app_detail.length == 0) {
        ...
        $.ajax({
            url: link_eproc + 'WA/ActionRequestWACreate',
            type: 'Post',
            data: {
                wa_number: txt_wa_number,
                dept_name: txt_dept_name,
...
function ActionRequestWAComplete() {
    ...
    var rs = $.xResponse(link_eproc + 'WA/ActionRequestWAComplete',
    {
        id: txt_id,
...
function ActionRequestWAApprove() {
    ...
    var rs = $.xResponse(link_eproc + 'WA/ActionRequestWAApprove',
    {
        id: txt_id,
        request_no: txt_request_no,
        actions: txt_actions,
...
function ActionRequestWAEdit() {
    ...
    if (app_detail.length == 0) {
        ...
        $.ajax({
            url: link_eproc + 'WA/ActionRequestWAEdit',
            type: 'Post',
            data: {
                id: txt_id,
                wa_number: txt_wa_number,
...
function ActionRequestWADelete() {
    ...
    $.ajax({
        url: link_eproc + 'WA/ActionRequestWADelete',
        type: 'Post',
        data: {
            id: txt_id,
            wa_number: txt_wa_number,
