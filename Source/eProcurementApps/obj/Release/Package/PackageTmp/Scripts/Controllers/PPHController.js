...
function BtnAction(action) {
    ...
    if (IsValidForm() == 0) {
        if (IsValidDate() == 0) {
            if (IsValidLength() == 0) {
                var id = $.xResponse(fullOrigin + '/IsInActive/', { value: $("#txt_pph_name").text() });
                if (id > 0) {
...
function CheckData(action) {
    txt_pph_name = $("#txt_pph_name").text();
    txt_id = 0;
    if (action == "edit") {
        txt_id = $("#txt_id").text();
    };
    $.ajax({
        ...
        success: function (data) {
            var field = document.getElementById('required_txt_pph_name');
            if (data == 1) {
