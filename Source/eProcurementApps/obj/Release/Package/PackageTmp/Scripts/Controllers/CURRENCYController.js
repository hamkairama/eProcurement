        ...
        if (IsValidDate() == 0) {
            if (IsValidLength() == 0) {
                if (IsOnlyNumberInput() == 0) {
                    var id = $.xResponse(fullOrigin + '/IsInActive/', { value: $("#txt_currency_name").text() });
                    if (id > 0) {
...
function CheckData(action) {
    txt_currency_name = $("#txt_currency_name").text();
    txt_id = 0;
    if (action == "edit") {
        txt_id = $("#txt_id").text();
    };
    $.ajax({
        ...
        data: {
            id: txt_id,
            currency_name: txt_currency_name,
        },
        cache: false,
        traditional: true,
        ...
        success: function (data) {
            var field = document.getElementById('required_txt_currency_name');
            if (data == 1) {
...
function ActionCreate() {
    txt_currency_name = $("#txt_currency_name").text();
    txt_rate = $("#txt_rate").text();
    txt_start_date = ConvertDate($("#txt_start_date").text());
    txt_convertion = $("#txt_convertion").text();
