...
function BtnAction(action) {
    ...
    if (IsValidForm() == 0) {
        if (IsValidDate() == 0) {
            if (IsValidLength() == 0) {
                var id = $.xResponse(fullOrigin + '/IsInActive/', { value: $("#txt_form_type_name").text() });
                if (id > 0) {
...
function CheckData(action) {
    txt_form_type_name = $("#txt_form_type_name").text();
    txt_id = 0;
    if (action == "edit") {
        txt_id = $("#txt_id").text();
    };
    $.ajax({
        ...
        data: {
            id: txt_id,
            form_type_name: txt_form_type_name,
        },
        cache: false,
        traditional: true,
        ...
        success: function (data) {
            var field = document.getElementById('required_txt_form_type_name');
            if (data == 1) {
...
function ActionCreate() {
    txt_form_type_name = $("#txt_form_type_name").text();
    txt_form_type_description = $("#txt_form_type_description").text();
    if ($("#cb_is_gt").is(':checked')) {
        txt_is_gt = 1;
    }
    else {
        txt_is_gt = 0;
    ...
    $.ajax({
        ...
        data: {
            form_type_name: txt_form_type_name,
            form_type_description: txt_form_type_description,
            is_gt: txt_is_gt,
        },
        cache: false,
        traditional: true,
